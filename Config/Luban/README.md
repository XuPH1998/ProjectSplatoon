...
五把主武器的历史参考修正见 [11.3.0 实施记录](../../Reports/Archive/2026-09-14/Docs/WeaponAudit/Weapon-Reference-Implementation.md)。当前源表仅保留功能字段，水平涂地范围通过弹道和碰撞测量，不再作为配置目标。`Tools/Luban/create-gameplay-tables.mjs <新目录>` 从现有源表复制完整模板，不再生成过时的单武器默认表，也不允许覆盖权威源表。

源数据按职责拆成四张强类型表。第 1 行为字段名，第 2 行为类型，第 3 行为中文说明及单位，第 4 行开始为数据。默认记录 ID 均为 1；字段名和资源地址保持英文稳定标识。

| 表 | 职责 |
| --- | --- |
| TbHero | 每行完整英雄配置：角色/武器模型、生命、移动恢复、发射、弹道、伤害、涂色和蓄力；当前 5 个英雄、71 个字段 |
| TbRoomMode | 默认英雄/场地、人数、时长、重生、友伤及待救泡泡规则；18 个字段 |
| TbMap | 地图 ID、名称、场景地址、归属网格；4 个字段 |
...
生成的 C#/JSON 禁止手工编辑。当前立体训练场 X 宽 32 米、Z 长 64 米，各可行走表面归属网格默认 0.125 米，最多 8 人，每队最多 4 人。地图尺寸和布局版本以场景 `PrototypeArena` 为准，不在配表重复填写；修改布局后仍须重新烘焙，场景拓扑签名继续参与联机一致性检查。计分由表面的 `Scores` 设置决定。模式以 `mapId` 引用 `TbMap`，配置加载地址为 `tbmap`。

`fireRate` 是射速的唯一配置，单位为有效发射次数/秒，冷却秒数为 `1 / fireRate`；霰弹一次齐射算一次，三连发组内使用该射速，组间恢复仍使用 `burstRecoveryFrames`，蓄力武器用它控制发射后的冷却。起手、蓄力和回墨锁定仍以 60Hz 参考帧计时。半自动点击缓存不能超过一次冷却，三连发组间恢复不能短于组内间隔，这些约束从射速换算，不需要额外填写间隔。

`characterGravity` 与 `projectileGravity` 分别控制角色和墨弹。墨弹重力允许为 0，表示没有重力下坠；负数和非有限数仍拒绝。`effectiveRange` 控制伤害有效射程，实际墨迹距离由弹道和碰撞决定。已移除 `fireIntervalFrames`、`paintRange`、`chargeMinPaintRange`、地图 `width/length/layoutVersion` 和 `groundOnlyScore`，旧迁移报告与历史测试数据只记录当时的配置。
