...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("Reports/SpecialWeapons/Network"))
    parser.add_argument("--scenarios", nargs="*", type=int, default=[0, 200])
    parser.add_argument("--input-cache-probe", action="store_true", help="Verify real client Q counter rebasing after respawn and loadout RPC")
    ...
    for index, rtt in enumerate(args.scenarios):
        ...
        probe_args = ["-specialInputProbe", directory / ("q-cache-" + uuid.uuid4().hex)] if args.input_cache_probe else []
        ...
        try:
            for role, port in [("host", host_port), ("client", proxy_port)]:
                ...
                players.append(hidden_process([args.player.resolve(), "-batchmode", "-force-d3d11",
                    "-screen-width", 960, "-screen-height", 540, "-specialRole", role,
                    "-specialPort", port, "-specialOutput", report,
                    "-logFile", directory / f"{role}.log", *probe_args], root))
        ...
        exit_codes = [p.returncode for p in players]
        passed = passed and exit_codes == [0, 0]
        if args.input_cache_probe:
            passed = passed and host.get("inputCacheProbe") == client.get("inputCacheProbe") == "True"
        results.append({"rtt_ms": rtt, "jitter_ms": jitter, "loss_percent": loss, "passed": passed,
                        "same_paint": same_paint, "exit_codes": exit_codes,
                        "input_cache_probe_requested": args.input_cache_probe, "host": host, "client": client})
