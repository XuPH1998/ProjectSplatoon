...
> 2026-09-14 修复补充：霰弹枪握持、恢复当帧姿势与飞行墨弹时钟问题已修复。173 项自动测试最终全部通过，并完成五武器／两队色的实际房主 GPU 检查。详见 [修复记录与运行截图](WeaponPresentationRepair/README.md)。以下保留原迁移阶段记录；尚未执行的真实多机、发布和性能项目仍未验收。
