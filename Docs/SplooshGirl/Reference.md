...
具体字段读取集中在 `SplooshGirlBuilder.BuildConfig`；六武器瞄准与涂墨覆盖值另由 `Tools/WeaponReference/rebuild_aim_ballistics.py` 记录。当前规则见 [弹道与瞄准设计](../AimBallistics/Design.md)。原始文件省略的继承值不作为“已证实原版值”。
...
8. 散布使用角度域幂分布：`theta = maxAngle * U^(log(bias)/log(.5))`，方位均匀。初速继承按人物 yaw 前向的有符号速度投影，忽略侧向 / 竖直分量。发射方向以武器参考距离构造，不随相机命中深度变化；主准星显示 6 帧内的首次接触点或中心弹道终点。此几何及概率分布均为项目重建，详见上方设计文档。
