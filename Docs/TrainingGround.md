...
| 玩法区域 | 15 个原有计分区域，101 个新增非计分平面区域；含墙面与掩体顶面 |
| 归属精度 | 各平面局部网格 0.125 米 |
...
出生区挡墙及前场、中央掩体的顶面直接使用可涂色混凝土，不再叠放固定粉色／蓝色盖板；顶面沿用已有的非计分涂色区域。

可行走面顶面与实体侧壁分别绑定。旧的单一 `Size` 配置已改为 `Width=32`、`Length=64`；场地布局版本为 4，墨水检查点格式为 5，玩家协议为 6。逐平面区域包含稳定 ID、原点、基底、尺寸、有效格与攀游开关，并参与检查点、增量墨迹和拓扑签名。边界、栏杆和桥底禁止攀游，内部竖直面与共面接缝可攀游。新增区域不参与计分。完整 CPU 归属网格及累计状态为 1,502,800 字节；未增加墨水 RT。训练场南侧新增 0/4/8/12/16 米标尺，属于无碰撞装饰。
...
四人队伍出生点按粉队 0–3、蓝队 0–3 绑定。保留原有 X=±3 米位置，新增 X=±6 米位置，Z 仍为两端 ±28 米。已有场景使用“喷墨对战/地图/增补四人队伍出生点”增补并更新拓扑签名，不重建地图布局。
