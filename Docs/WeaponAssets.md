...
武器 Inspector 顶部集中显示资源引用、发射模式、弹道运动模式、枪口模式及规则开关；通用参数按发射、弹道、伤害和涂色分组显示。专用参数随当前枚举筛选：三连发与泡泡连发显示各自组周期，蓄力枪和旋转枪显示适用的蓄力端点，弹跳泡泡、漂浮泡泡、普通双枪、爆破枪和爆炸泼桶显示对应参数。启用参考规则后显示参考参数，隐藏已被替代的旧参数；射手详细参数随其模式和开关显示。切换模式只改变显示，不清空已配置的数值。多选资产时只显示所有选中武器都适用的字段，专用参数可单独选择资产调整。
