...
霰弹由 2°/4°扩至原有 5°/10°并恢复到基础值。月兔改用固定地面 0° / 空中 8° 散布；旧蓄力收紧逻辑由专用测试夹具保留。
...
合法候选作为一份完整不可变配置在模拟步边界替换。调试房是本地 Host；不向联网房广播实时资产修改。普通入房内容签名包含全部武器参数和资产路径。当前玩家协议为 29，武器模拟版本为 9（后续以 `PlayerSnapshot.ProtocolVersion` 和 `GameplayContentSignature.WeaponSimulationVersion` 为准）。散布进度、双轴角度、发射状态、泡泡组内剩余发数及以双精度秒表示的蓄力状态参与预测和网络校正；泡泡反射段和终止时间同步到客户端。
