...
{
    [InitializeOnLoad]
    public sealed class ExplosherValidationRunner : ICallbacks
    {
        const string Root = "Reports/Explosher", Running = "Explosher.Running", Label = "Explosher.Label";
        static TestRunnerApi api;
        static double next;
        static ExplosherValidationRunner()
        {
            api = ScriptableObject.CreateInstance<TestRunnerApi>(); api.RegisterCallbacks(new ExplosherValidationRunner());
        ...
        static void Poll()
        {
            ...
            const string request = "Temp/Explosher/tests";
            if (!File.Exists(request)) return;
            string token = File.GetLastWriteTimeUtc(request).Ticks + ":" + File.ReadAllText(request);
            if (SessionState.GetString("Explosher.Refreshed", "") != token)
            { SessionState.SetString("Explosher.Refreshed", token); AssetDatabase.Refresh(); return; }
