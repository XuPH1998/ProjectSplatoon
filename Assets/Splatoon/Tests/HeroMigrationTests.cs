...
namespace Splatoon.Tests
{
    public sealed class HeroMigrationTests
    {
        ...
        HeroContent Content(int id, GameObject character = null, GameObject weapon = null) => new(GameplayConfig.GetHero(id), character ?? AssetDatabase.LoadAssetAtPath<GameObject>(CharacterPath), weapon ?? AssetDatabase.LoadAssetAtPath<GameObject>(WeaponPath));
        static PlayerSnapshot State(int hero) => new() { HeroId = hero, Health = 100, Ink = 100, Team = 1, Grounded = true, Revision = 4 };

        [Test] public void AllHeroFieldsMatchIndependentPreMigrationRecords()
        {
            ...
            for (int i = 0; i < 5; i++) foreach (var item in mapping.Children)
            {
                ...
                else Assert.That(value.Value, Is.EqualTo(expected.Value));
            }
        }
        [Test] public void SelectedHeroDrivesMovementRecoveryAndDisplay()
        {
            ...
            Assert.That(details.Single(x => x.label == "满墨发数").value, Is.EqualTo("120 发"));
        }
        ...
        [Test] public void BinderReusesSharedModelsAndReplacesAlternativeModelsWithoutLeftovers()
        {
            ...
            for (int id = 2; id <= 5; id++) { Assert.That(binder.Apply(Content(id)), Is.False); Assert.That(binder.Visual, Is.SameAs(first)); }
            ...
            for (int i = 0; i < 3; i++)
            {
                Assert.That(binder.Apply(Content(2, alternateCharacter, alternateWeapon)), Is.True);
        ...
        {
            var source = new Assets(); source.Values["Character/RifleGirl"] = AssetDatabase.LoadAssetAtPath<GameObject>(CharacterPath);
            source.Values["Weapon/RifleGirlRifle"] = AssetDatabase.LoadAssetAtPath<GameObject>(WeaponPath);
            ...
            Assert.That(source.Loads, Is.EqualTo(2)); Assert.That(service.AssetCount, Is.EqualTo(2)); Assert.That(service.All.Count(), Is.EqualTo(5));
        ...
        {
            Load(rows => { rows[1]["characterPrefabAddress"]="Test/AlternateCharacter"; rows[1]["weaponPrefabAddress"]="Test/AlternateWeapon"; });
            ...
            var assets=new Assets();assets.Values["Character/RifleGirl"]=character;assets.Values["Weapon/RifleGirlRifle"]=weapon;
            ...
            Assert.That(assets.Loads,Is.EqualTo(4));
        ...
        [Test] public void SignatureIncludesNonDefaultHeroPresentationAndIsIndependentOfEnumerationOrder()
        {
            ...
            var heroes = new[] { Content(1), Content(2, character) };
