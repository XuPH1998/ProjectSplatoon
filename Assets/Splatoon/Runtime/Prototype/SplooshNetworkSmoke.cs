...
namespace Splatoon.Prototype
{
    ...
    public sealed class SplooshNetworkSmoke : MonoBehaviour
    {
        ...
        readonly Dictionary<string, string> combatStates = new();
        static string Arg(string key, string fallback) => HeroSelectionSmoke.Arg(key, fallback);
        // Reuse the opt-in independent-player scenario for either replaced main weapon.
        static int TargetHero => int.Parse(Arg("-splooshHero", "8"));
        ...
        void Update()
        {
            ...
            if (age >= 52 && age < 56 || age >= 62 && age < 67)
                foreach (var other in PrototypePlayer.ByOwner.Values)
                {
                    var observed = other.Snapshot.Value;
                    string key = FormattableString.Invariant($"{other.OwnerClientId}/{observed.Revision}/{observed.HeroRevision}/{observed.SimulatedAt:R}");
                    combatStates[key] = FormattableString.Invariant($"{observed.HeroId},{observed.Ink:R},{observed.Health:R},{observed.ShotSequence},{observed.SplatlingReservedInk:R},{observed.SplatlingRemaining}");
                }
            players = Math.Max(players, PrototypePlayer.ByOwner.Count);
            if (s.Revision == life && s.HeroId == TargetHero && s.ShotSequence > sequence) emitted += (int)(s.ShotSequence - sequence);
            sequence = s.ShotSequence; life = s.Revision;
            int desired = age >= 20 && age < 26 ? 1 : TargetHero;
            ...
            tall |= age >= 20 && s.HeroId == 1; smallAgain |= tall && s.HeroId == TargetHero;
            paper |= s.HeroId == TargetHero && s.ShowsSwimBody && p.SwimBody.FlatHitActive;
            remoteSmall |= PrototypePlayer.ByOwner.Values.Any(other => other != p && other.Snapshot.Value.HeroId == TargetHero &&
                Mathf.Abs(other.SwimBody.CapsuleHitVolume.height - Splatoon.Config.GameplayConfig.GetHero(TargetHero).StandingHeight) < .001f);
            ...
            if (host && (age >= 52 && !combatPlaced || age >= 53.8 && !secondDuel))
            {
                ...
                foreach (var other in match.Players)
                {
                    ...
                    // A released splatling keeps firing; each duel must start with an empty action queue.
                    WeaponSimulation.Cancel(ref fight, new PlayerInputFrame { FireSequence = fight.ConsumedFire, ReleaseSequence = fight.ConsumedRelease });
                    ...
                    var look = FightLook(other, fight); fight.Yaw = fight.BodyYaw = look.x; fight.Pitch = look.y;
                    fight.Velocity = fight.PlanarVelocity = Vector3.zero; fight.VerticalSpeed = 0;
            ...
            died |= s.Health <= 0; respawned |= died && s.Health > 0 && s.HeroId == TargetHero;
            if (age > 72 && !captured)
            ...
            if (s.HeroId == TargetHero && age > 10 && age < 19)
            {
        ...
        public static void ModifyInput(PrototypePlayer player, ref PlayerInputFrame input)
        {
            ...
            input.Fire = !player.HeroChangePending && player.PresentedState.HeroId == TargetHero &&
            ...
            if (age >= 52 && age < 56) input.Look = FightLook(player, player.PresentedState);
        ...
        // Solve the fixture's look against the actual weapon-reference launch, including muzzle offset.
        // The two-metre lane lies within both weapons' straight phase; no gameplay aim is changed.
        static Vector2 FightLook(PrototypePlayer player, PlayerSnapshot state)
        {
            var w = Splatoon.Config.GameplayConfig.GetWeapon(TargetHero);
            var target = PrototypeArena.Spawn(1, 0) + Vector3.forward * (state.Team == 1 ? 2 : 0)
                + Vector3.up * (Splatoon.Config.GameplayConfig.GetHero(TargetHero).StandingHeight * .5f);
            var look = new Vector2(state.Team == 1 ? 0 : 180, 0);
            for (int i = 0; i < 8; i++)
            {
                var rotation = Quaternion.Euler(look.y, look.x, 0);
                var pivot = state.Position + PrototypePlayer.CameraPivotOffset(state, player.Presentation);
                var camera = PrototypePlayer.CameraPosition(pivot, rotation, player.Presentation);
                var muzzle = state.Position + Quaternion.Euler(0, look.x, 0) * player.MuzzleOffset(look.y);
                var aim = WeaponLaunch.Geometry(camera, rotation * Vector3.forward, muzzle, w, 1);
                var desired = (target - muzzle).normalized;
                look.x += Mathf.DeltaAngle(Mathf.Atan2(aim.InitialDirection.x, aim.InitialDirection.z) * Mathf.Rad2Deg,
                    Mathf.Atan2(desired.x, desired.z) * Mathf.Rad2Deg);
                look.y += (Mathf.Asin(aim.InitialDirection.y) - Mathf.Asin(desired.y)) * Mathf.Rad2Deg;
            }
            return look;
        }
        ...
        async void Finish(string error)
        {
            ...
            File.WriteAllLines(path + ".states.tsv", combatStates.Select(row => row.Key + "\t" + row.Value));
