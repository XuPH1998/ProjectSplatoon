...
namespace Splatoon.Combat
{
    ...
    public sealed class InkPresentation : MonoBehaviour
    {
        ...
        private readonly Dictionary<ParticleSystem, ParticleSystem[]> _impactChildren = new();
        ...
        private readonly List<(ParticleSystem effect, float until)> _effects = new(96);
        private void Awake()
        {
            Current = this;
            ...
            for (int i = 0; i < 96; i++) { var effect = Instantiate(ImpactPrefab, transform); _impactChildren[effect] = effect.GetComponentsInChildren<ParticleSystem>(true); effect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear); effect.gameObject.SetActive(false); _pool.Enqueue(effect); }
        }
        ...
        public void Impact(InkImpact impact)
        {
            ...
            effect.transform.SetPositionAndRotation(impact.Position + impact.Normal * .02f, impact.Normal.sqrMagnitude > .01f ? Quaternion.LookRotation(impact.Normal) : Quaternion.identity);
            foreach (var particle in _impactChildren[effect]) { var main = particle.main; main.startColor = PrototypeArena.TeamColor(impact.Team); }
            effect.Play(false); _effects.Add((effect, Time.time + 1.3f));
        }
        private void LateUpdate()
        {
            ...
            for (int i = _effects.Count - 1; i >= 0; i--)
                if (Time.time >= _effects[i].until) { var effect = _effects[i].effect; effect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear); effect.gameObject.SetActive(false); _pool.Enqueue(effect); _effects.RemoveAt(i); }
        }
        public void Clear()
        {
            _shots.Clear();
            ...
            foreach (var pair in _effects) { pair.effect.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear); pair.effect.gameObject.SetActive(false); _pool.Enqueue(pair.effect); }
            _effects.Clear();
        }
