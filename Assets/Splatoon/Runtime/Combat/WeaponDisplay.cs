...
namespace Splatoon.Combat
{
    ...
    {
        public static string Mechanism(cfg.HeroConfig w) => w.FireMode switch { 4 => "旋转枪 / 两段蓄力连射", 1 => "三连发 / 长按连续", 2 => "蓄力 / 松开发射", 3 => w.MuzzleMode == 1 ? "半自动 / 右左交替" : w.PelletCount > 1 ? "半自动 / 霰弹齐射" : "半自动 / 点击或长按射击", _ => "全自动" };
        public static string Damage(cfg.HeroConfig w) => WeaponSimulation.IsSplatling(w) ? $"未满蓄 {w.ChargePartialMaxDamage:0} / 满蓄 {w.Damage:0} → 远端 {w.DamageMin:0}" : WeaponSimulation.IsCharge(w) ? $"{w.ChargeMinDamage:0}–{w.ChargePartialMaxDamage:0} / 满蓄 {w.Damage:0}" : w.PelletCount > 1 ? $"{w.PelletCount} × {w.Damage:0}→{w.DamageMin:0}" : $"{w.Damage:0} → {w.DamageMin:0}";
        ...
        public static string Ink(cfg.HeroConfig w) => WeaponSimulation.IsSplatling(w) ? $"满蓄 {w.ShotInk*SplatlingSimulation.Rounds(w,w.ChargeFrames):0.##} / {SplatlingSimulation.Rounds(w,w.ChargeFrames)} 发，取消返还未发射部分" : WeaponSimulation.IsCharge(w) ? $"{w.ChargeMinInk:0.##} / 满蓄 {w.ShotInk:0.##}" : $"{w.ShotInk:0.##} / {(w.PelletCount > 1 ? "次齐射" : "发")}";
        public static string Range(cfg.HeroConfig w) => WeaponSimulation.IsCharge(w) || WeaponSimulation.IsSplatling(w) ? $"{w.ChargeMinRange:0.#}–{w.EffectiveRange:0.#} 米" : $"{w.EffectiveRange:0.#} 米";
