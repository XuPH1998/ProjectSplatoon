...
namespace Splatoon.Combat
{
    ...
    {
        public static string Mechanism(cfg.HeroConfig w) => w.FireMode switch { 1 => "三连发 / 长按连续", 2 => "蓄力 / 松开发射", _ => "全自动" };
        public static string Damage(cfg.HeroConfig w) => WeaponSimulation.IsCharge(w) ? $"{w.ChargeMinDamage:0}–{w.ChargePartialMaxDamage:0} / 满蓄 {w.Damage:0}" : $"{w.Damage:0} → {w.DamageMin:0}";
        ...
        public static string Ink(cfg.HeroConfig w) => WeaponSimulation.IsCharge(w) ? $"{w.ChargeMinInk:0.##} / 满蓄 {w.ShotInk:0.##}" : $"{w.ShotInk:0.##} / 发";
