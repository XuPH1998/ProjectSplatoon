...
namespace Splatoon.Combat
{
    ...
    public sealed class InkFlightPresentation : IDisposable
    {
        ...
        readonly InkFlightProfile _profile;
        ...
        sealed class Group
        {
            ...
            public Group(ParticleSystem prefab, Transform parent)
            {
                ...
                System.name = "Ink flight lane"; System.gameObject.layer = InkFlightProfile.Layer;
                ...
                var go = new GameObject("Ink flight ribbon"); go.layer = InkFlightProfile.Layer;
        ...
        public InkFlightPresentation(Transform parent, ParticleSystem prefab, InkFlightProfile profile)
        {
            _profile = profile;
            for (int i = 0; i < GroupLimit; i++) { _groups[i] = new Group(prefab, parent); _streams[i] = _groups[i].System; }
        }
        ...
        float Lifetime => _profile != null ? _profile.VisualLifetime : 1;
        float Rate => _profile != null ? _profile.ParticlesPerBurst / _profile.BurstInterval : 2 / .03f;
        ...
        public bool Spawn(InkShot shot, double now)
        {
            ...
            float wanted = continuous ? Rate / Mathf.Max(1, w.FireRate) + group.DensityRemainder : 2;
            int count = Mathf.Clamp(Mathf.FloorToInt(wanted), 2, 16);
            ...
            for (int i = _shotCount - 1; i >= 0; i--)
            {
                ...
                for (int n = 0; n < visual.Count; n++)
                {
                    ...
                    if (n != 0)
                    {
                        ...
                        float spread = (_profile != null ? _profile.SatelliteSpread : .035f) * Mathf.Min(1, (float)t * 8);
        ...
        void BuildRibbon(Group g, Camera camera)
        {
            ...
            for (int i = 0; i < g.Count; i++)
            {
                ...
                if (i == 0 || g.Segments[i] != g.Segments[i - 1] || Vector3.Distance(p.position, g.Particles[i - 1].position) > (_profile != null ? _profile.MaxRibbonGap : 1.25f)) continue;
                int a = (i - 1) * 2;
